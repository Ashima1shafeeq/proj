...
function MessOff() {
  ...
  const approve = (id) => {
    ...
    setApprovedReqs((prev) => prev + 1); // Increase accepted count
    updateRequest(id, "approved");
  };

  const reject = (id) => {
    setNewReqs((prev) => prev.filter((req) => req.id !== id));
    setRejectedReqs((prev) => prev + 1); // Increase rejected count
  ...
  return (
    <div className="w-full h-screen flex flex-col gap-3 items-center justify-center">
        ...
        <Bar
          data={{
            ...
            datasets: [
              {
                label: "Requests",
                data: [approvedReqs, rejectedReqs, newReqs.length],
      ...
      <div className="bg-neutral-950 px-10 py-5 rounded-xl shadow-xl sm:w-[50%] sm:min-w-[450px] w-full mt-5 max-h-96 overflow-auto">
        ...
        <ToastContainer position="top-right" autoClose={3000} hideProgressBar={false} draggable theme="dark" />
      </div>
    </div>
  );
}
